...
export function DashboardPage() {
  ...
  const handleToggleSaved = (companyId: string) => {
  ...
  return (
    <div className="bg-background text-foreground min-h-screen">
      {/* Header with search bar and logout */}
      <Header 
        showSearch 
      ...
      <main className="px-6 py-12">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-4xl lg:text-5xl font-serif">My Dashboard</h1>
            {!isLoading && (
              <p className="text-muted-foreground">
                {savedCompanyIds.size} saved | {companies.length} {companies.length === 1 ? 'service' : 'services'} found
              </p>
            )}
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {companies.map((company, index) => (
                  ...
                  >
                    <CompanyCard 
                      company={company}
                      isSaved={savedCompanyIds.has(company.id)}
                      onToggleSaved={() => handleToggleSaved(company.id)}
                    />
                  </div>
                ))}
              </div>
